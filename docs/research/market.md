...
2. 이 중 "아이디어·자료를 텍스트+이미지로 체계적으로 기록"하려는 비율을 10~20%로 가정 (PKM 습관은 소수 — Obsidian MAU 150만 vs Notion 1억+ 비율 참조) → 250만~3,000만 × 10~20% ≈ 300만~600만 명.
3. 금액 환산은 앱 구독 없음 전제에서 1회 구매 $10 가정 (Procreate $12.99 선례 — 창작자 시장에서 1회 구매 $10대가 검증된 가격대).

### SOM 추정 로직 (가정 체인)
1. Obsidian MAU 약 150만 중 미술·비주얼 창작 워크플로 사용자 비율을 3~5%로 가정 (커뮤니티 플러그인 중 이미지/캔버스 계열 비중 기준의 감각적 추정) → 4.5만~7.5만 명.
