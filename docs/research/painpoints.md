...
**시사점:** 이미지를 텍스트 노트와 동급으로(이른바 "1급 시민", first-class citizen) 다루는 것(갤러리 뷰, 이미지 속성)이 미술용 위키의 차별점이 된다.
...
**시사점:** 저장은 번거로움 없이 바로(일단 던져넣기), 정리는 나중에 가볍게 할 수 있는 인박스형 흐름이 필요하다.
...
1. **이미지를 텍스트와 동급으로 다뤄야 한다.** 조사된 페인포인트의 절반(1·2·3번)이 "텍스트 노트 도구에서 이미지가 뒷전으로 밀리는 것"에서 나온다. ArtObsidian은 이미지 첨부에 태그·메모·검색을 붙이고 갤러리로 훑을 수 있어야 한다.
2. **저장은 최대한 간단하게 + 정리는 나중에.** 저장 시 분류를 강요하면 한 폴더 덤핑으로 귀결된다. 인박스에 던져넣고, 태그·위키링크·유사도 추천으로 나중에 연결하는 흐름이 맞다.
3. **개방 포맷은 방어가 아니라 공격 포인트.** 다른 도구로 옮기다 자료가 깨졌던 나쁜 경험(4번, 이른바 락인 트라우마)이 실제로 있으므로 "평문 md + 옵시디언 호환"을 전면에 내세울 가치가 있다.
4. **구조는 쓰다 보면 자연스럽게 자라나게.** 엄격한 분류 체계 대신 링크·태그·관련 노트로 구조가 자라나게 하는 현재 방향(그래프 뷰, 유사도)이 5번 페인포인트와 잘 맞아떨어진다.
